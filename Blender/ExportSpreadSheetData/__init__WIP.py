...
def get_evaluated_geometry(context):
    """Return the evaluated GeometrySet of the active object."""
...
class ESD_AttributeItem(bpy.types.PropertyGroup):
    """defines the individual stored-attribute entries used by the UI."""
...
def make_attribute_field(attribute):
    """Create an export field from a Blender Attribute"""
...
# ====================================================
def build_export_spec(context, mesh, attribute_domain):
...
class SPREADSHEET_OT_refresh_attributes(bpy.types.Operator):
    """Refresh Attributes button/operator
    gets the evaluated Mesh through mesh.py
    discovers attributes and populates scene.esd_stored_attributes"""
    ...
    def execute(self, context):
        evaluated_mesh = mesh.get_evaluated_mesh(context)
        if evaluated_mesh is None:
            ...
            return {"CANCELLED"}
        evaluated_attributes = mesh.get_mesh_attributes(evaluated_mesh)
        ...
        self.report(
            {"INFO"},
            f"Found {len(evaluated_attributes)} stored attributes",
        )
        return {"FINISHED"}
...
class SPREADSHEET_OT_export_geometry_data(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        domain_name = domain_names.get(
            geometry_domain,
            geometry_domain.lower(),
        )
        filename = f"{obj.name}_{domain_name}_data.csv"
    ...
    def execute(self, context):
        ...
        attribute_domain = mesh.get_attribute_domain(
            geometry_domain,
        )
        # ----------------------------------
        # Build fields
        ...
        # ----------------------------------
        try:
            elements = mesh.get_geometry_elements(
                evaluated_mesh,
                geometry_domain,
            )
        except Exception as exc:
    ...
    def draw(self, context):
        ...
        try:
            ...
            visible_attributes = [
                ...
                if attr.domain == attribute_domain
            ]
