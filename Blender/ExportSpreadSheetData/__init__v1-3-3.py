...
def get_mesh_attributes(mesh):
    ...
    for attr in mesh.attributes:
        ...
        # Ignore attributes handled elsewhere.
        if attr.name in EXCLUDED_ATTRIBUTES:
            continue
...
class SPREADSHEET_OT_export_geometry_data(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # -------------------------------
        # Check selected stored attributes
        # -------------------------------

        selected_attributes = [
            attr
            for attr in context.scene.esd_stored_attributes
            if attr.selected
            and (
                (geometry_domain == "VERTEX" and attr.domain == "POINT")
                or (geometry_domain == "EDGE" and attr.domain == "EDGE")
            )
        ]
        ...
        if not (export_index or export_position or selected_attributes):
            self.report(
                {"ERROR"},
